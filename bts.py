...
class ReqClient(object):
    ...
    def getPage(self, urls, charset='utf8'):
        ...
        """
        btsFlag = "/tmp/btscnt"
        if os.path.exists(btsFlag) == True:
            f = open(btsFlag, "r+")
            cnt = f.read()
            if int(cnt) == 0:
                f.close()
    ...
    def __getPage(self, url, name, charset):
	    ...
	    res = OrderedDict()
	    ## get the high's bid.
	    idx = re.search("High:\ ", docc, flags=0).start()
	    f.seek(idx+6, 0)
            bidstr = f.read(6)
            if bidstr[5] != "<":
                bidH = float(bidstr)
	    ...
	    ## get the Low's bid.
	    idx = re.search("Low:\ ", docc, flags=0).start()
	    f.seek(idx+5, 0)
            bidstr = f.read(6)
            if bidstr[5] != "<":
                bidL = float(bidstr)
	    ...
	    res["Nam"] = name
            ...
            #logger.infoLog(res)
            #if (name=="lrc") and (bidC>1.2 or bidC<0.82):
...
if __name__ == '__main__':
    '''
        {"url": "https://gate.io/trade/LRC_USDT", "name": "lrc"},
        {"url": "https://gate.io/trade/AE_USDT", "name": "ae"},
        {"url": "https://gate.io/trade/BTS_USDT", "name": "bts"},
    '''
    urls = [
        {"url": "https://gate.io/trade/LRC_USDT", "name": "lrc"},
        {"url": "https://gate.io/trade/AE_USDT", "name": "ae"},
